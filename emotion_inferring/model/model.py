...
class Model_Creator(object):
    ...
    def _create_network(self,
        ...
        if self.hparams.acoustic_enable:
            if self.hparams.CNN_extractor:
                acoustic_features = Stack_CNN_blocks(acoustic_features, self.hparams.self_attention, is_training)

            ## Acoustic representation generation
            if self.hparams.global_attention:
                speech_representation, temp  = GCA_LSTM(acoustic_features, self.hparams.units,
                                                  self.hparams.GCA_iterations, 'SPEECH', is_training)
                ...
                speech_representation = LSTM_2_layers(acoustic_features, self.hparams.units, 'SPEECH')
                speech_representation = speech_representation[:, -1]

        if self.hparams.text_enable:
            ## Textual representation generation
            textual_representation = BLSTM_2_layers(textual_features, 128, 'TEXT', is_training)
        ...
        if self.hparams.acoustic_enable and self.hparams.text_enable:
            ...
            merged_representation = tf.concat([speech_representation, textual_representation], axis=1)
        ...
        if presentation_output:
            ...
            # Dense layer
            pre_logits = output_layer(merged_representation, target_class_dim, is_training)
            return pre_logits, temp[:,2:]


    def inference(self, acoustic_features, textual_features, emotion_targets = None, target_class_dim = None,
                  l2_regularization_strength=None, is_training = True, predicting = False):
        with tf.name_scope('ER_model'):
            ...
            pred_logits, temp     = self._create_network(acoustic_features, textual_features,
                                                         target_class_dim, is_training)
            if predicting:
                return pred_logits
            else:
                with tf.name_scope('loss_acc'):
                    xent = tf.nn.softmax_cross_entropy_with_logits_v2(logits=[pred_logits],
    ...
    def representation(self, acoustic_features, textual_features):
        with tf.name_scope('ER_model'):
            acoustic_features = tf.expand_dims(acoustic_features, axis=0)
            textual_features  = tf.expand_dims(textual_features, axis=0)
            speech_represenation = self._create_network(acoustic_features= acoustic_features,
