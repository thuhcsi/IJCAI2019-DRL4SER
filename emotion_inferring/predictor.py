...
class emotion_predictor(object):
    def __init__(self,
        ...
        with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
            with tf.device('/cpu:0'):
                ...
                self.input_acous = tf.placeholder(dtype=tf.float32, shape=(None, self.hparams.condition_num),
                                                  name='input_acoustic_features')
                self.input_texts = tf.placeholder(dtype=tf.float32, shape=(None, 300), name='input_textual_features')
                if not presentation_output:
                    self.predict_logits = self.net.inference(acoustic_features=self.input_acous,
                                                             textual_features =self.input_texts,
                                                             target_class_dim = 4,
                                                             is_training=False, predicting = True)
                    self.predict_class = tf.nn.softmax(tf.cast(self.predict_logits, tf.float64))
                    self.predict_class = tf.argmax(self.predict_class, axis=-1, name='predicted_class')
                elif presentation_output:
                    self.generated_representation = self.net.representation(acoustic_features=self.input_acous,
                                                                            textual_features =self.input_texts)
                    self.generated_representation = tf.reshape(self.generated_representation, [-1],
                                                               name='speech_representation')
                else:
        ...
        if convert_to_pb:
            ...
            target_save_dir = os.path.dirname(os.path.realpath(__file__)) + '/' + pb_save_dir
            os.makedirs(target_save_dir, exist_ok=True)
            output_grap = target_save_dir +  pb_name
            print('Saving .PB in : ' + output_grap)
            output_grap_def = tf.graph_util.convert_variables_to_constants(self.sess,
        ...
        self.mel_min, self.mel_max = np.load(self.checkpoint + '/mel_min_max_var.npy')
        self.Word2Vec = KeyedVectors.load_word2vec_format(hparams.word2vec_path, binary=True)

    def load(self, saver, sess, checkpoint):
        print("Trying to restore saved checkpoints from {} ...".format(checkpoint), end="")
    ...
    def inferring(self, filename):
        ...
        acous, text = self.input_gen(filename=os.path.join(self.sample_dir, filename))
        print('Inferring emotion in '+ filename + ' ......')
        output = self.sess.run(self.predict_class,
                               feed_dict={self.input_acous: acous,
    ...
    def input_gen(self, filename):
        ...
        acoustic_features = acoustic_gen(self.hparams, audio, mel_max = self.mel_max, mel_min = self.mel_min)
