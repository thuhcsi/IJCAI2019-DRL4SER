...
def train(args, hparams):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.name_scope('create_inputs'):
            ...
            input_valid, text_valid, target_valid = reader.dequeue_valid(args.batch_size)
    ...
    clipped_gradients, norm = tf.clip_by_global_norm(gradients, hparams.gradients_limit)
    loss_train_op = optimizer.apply_gradients(zip(clipped_gradients, variables),global_step=tf_global_step)
    ...
    # Validation
    with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
        loss_valid, acc_valid, valid_true, valid_pred, GCA_score = net.inference(acoustic_features=input_valid,
    ...
    with tf.Session() as sess:
        ...
        tf_global_step = tf.assign(ref=tf_global_step,value=tf.constant(last_saved_step))
        ...
        try:
            for step in range(saved_global_step + 1, args.num_steps):
                ...
                print('step {:d} - loss = {:.3f} acc = {:.3f}, lr = {:.6f} ({:.3f} sec/step)'
                ...
                if step % args.checkpoint_every == 1:
                    ACC, F1, y_true, y_pred = validate(args.batch_size, sess, loss_valid,
                                                       acc_valid, valid_true, valid_pred, GCA_score,
                                                       summaries, writer, step, std_out= False)
                    if ACC > Acc_save:
                        Acc_save = ACC
                        f1_save = F1
                        L_save = y_true
                    ...
                    print("Precision", sk.metrics.precision_score(L_save, P_save, average='macro'))
            ...
            _, _, _, _ = validate(args.batch_size, sess, loss_valid,
                                               acc_valid, valid_true, valid_pred, GCA_score,
                                               summaries, writer, step, std_out=True)
            print("Best Accuracy", Acc_save)
            print("Precision", sk.metrics.precision_score(L_save, P_save, average='macro'))
            print("Recall", sk.metrics.recall_score(L_save, P_save, average='macro'))
            print("f1_score", f1_save)
...
def validate(batch_size, sess, loss_op, acc_op, label_op, pred_op, GCA_score_op, summaries_op, writer, step, std_out):
