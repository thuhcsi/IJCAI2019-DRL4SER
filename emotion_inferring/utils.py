...
def validate_directories(args):
    ...
    logdir = args.logdir
    if logdir is None:
        logdir = get_default_logdir(logdir_root,"{0:%Y-%m-%dT%H-%M-%S}".format(datetime.now()))
...
def learning_rate_decay(global_step, args, hparams):
    ...
    learning_rate = tf.cond(
        ...
        lambda: tf.train.exponential_decay(args.learning_rate,
                                           global_step - warm_up_step + 1,
                                           decay_step, 0.5))
...
def create_optimizer(global_step,args, hparams):
    with tf.variable_scope('optimizer'):
        ...
        optimizer = optimizer_factory[args.optimizer](learning_rate=learning_rate_decayed,
