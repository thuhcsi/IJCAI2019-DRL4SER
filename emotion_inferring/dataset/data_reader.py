...
class DataReader(object):
  ...
  def __init__(self, hparams, coord, logdir, renew=False, queue_size=1024):
    ...
    self.queue_train = tf.PaddingFIFOQueue(
        queue_size,
        dtypes=['float32', 'float32', 'int32'],
        shapes=[(self.padding, input_dim), (None, 300), (1, 1)])
    self.enqueue_train = self.queue_train.enqueue(
        [self.input_placeholder, self.text_placeholder, self.class_placeholder])
    self.queue_valid = tf.PaddingFIFOQueue(
        1024,
        dtypes=['float32', 'float32', 'int32'],
        shapes=[(self.padding, input_dim), (None, 300), (1, 1)])
    self.enqueue_valid = self.queue_valid.enqueue(
        [self.input_placeholder, self.text_placeholder, self.class_placeholder])

    normalization_initial(self.train_data, self.hparams, logdir)

    if self.padding != None:
      self.generator_train = aoustic_features_generator(self.train_data,
                                                        self.hparams,
                                                        self.padding)
      self.generator_valid = aoustic_features_generator(self.valid_data,
                                                        self.hparams,
                                                        self.padding)
    else:
      self.generator_train = aoustic_features_generator(self.train_data,
  ...
  def start_threads_train(self, sess, n_threads=1):
    for _ in range(n_threads):
      thread_train = threading.Thread(target=self.thread_main_train,
                                      args=(sess,))
  ...
  def start_threads_valid(self, sess, n_threads=1):
    for _ in range(n_threads):
      thread_valid = threading.Thread(target=self.thread_main_valid,
                                      args=(sess,))
