...
def split_wav(wav, features, emotions):
    ...
    for ie, e in enumerate(emotions):
        ...
        e['acoustic_features'] = features[int(start * framerate //shift): int(end * framerate//shift)]
        frames.append({'left': e['left'], 'right': e['right'], 'acoustic_features': e['acoustic_features']})
    return frames
...
def pad_sequence_into_array(Xs, maxlen=None, truncating='post', padding='post', value=0.):
    Nsamples = len(Xs)
    if maxlen is None:
        lengths = [s.shape[0] for s in
                   Xs]  # 'sequences' must be list, 's' must be numpy array, len(s) return the first dimension of s
        maxlen = np.max(lengths)

    Xout = np.ones(shape=[Nsamples, maxlen] + list(Xs[0].shape[1:]), dtype=Xs[0].dtype) * np.asarray(value,
                                                                                                     dtype=Xs[0].dtype)
